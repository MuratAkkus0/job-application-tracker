...
function DroppableColumn({
  ...
  const sortedJobApplications = column.jobApplications.sort(
    (a, b) => a.order - b.order,
  );
  ...
  return (
    <Card className="min-w-[300px] shrink-0 shadow-md p-0">
      ...
      >
        <SortableContext
          items={sortedJobApplications.map((job) => job._id)}
          strategy={verticalListSortingStrategy}
        >
          {sortedJobApplications.map((job, key) => (
            <SortableJobCard
              key={key}
...
function SortableJobCard({
  ...
  const {
    ...
    transform,
    isDragging,
  } = useSortable({ id: job._id, data: { type: "job", job } });
